...
class BaseBSDEPricer(tf.keras.Model):
    ...
    def loss_fn(self, data: Tuple[tf.Tensor], training=None) -> tf.Tensor:
        loss_interior, loss_tml = self.loss_interior(data, training), self.loss_terminal(data, training) 
        loss = self.alpha * self.loss_interior(data, training) + self.loss_terminal(data, training) 
