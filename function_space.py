...
class BlackScholesFormula(tf.keras.Model):
    ...
    def call(self, inputs: Tuple[tf.Tensor], training=None) -> tf.Tensor:
        ...
        d1 = (tf.math.log(x / k) + (r + vol ** 2 / 2) * (T - time_tensor)) / (vol * tf.math.sqrt(T - time_tensor) + 0.000001)
...
class DeepONet(tf.keras.Model):
    ...
    def __init__(self, branch_layer: List[int], trunk_layer: List[int], activation: Optional[str]):
...
class DeepONetwithPI(DeepONet):
    ...
    def reshape_state(self, state: tf.Tensor):
        ...
        return tf.reshape(state, [-1, state.shape[1], state.shape[2], self.num_assets, num_markov])
...
class DeepKernelONetwithoutPI(DeepONet):
    def __init__(self, branch_layer: List[int], 
        ...
        super(DeepKernelONetwithoutPI, self).__init__(branch_layer, trunk_layer, activation)
        if dense:
