...
from solvers import BaseBSDESolver, BermudanSolver, EuropeanSolver, FixIncomeEuropeanSolver
...
class BSDETrainer:
    ...
    def train(self, nr_epochs: int, dataset_path: str, checkpoint_path: str):
        self.pre_setting()
        # Create a callback that saves the model's batch loss
        
        history = LossHistory()
        # load dataset
        samples = self.eqn_config.sample_size
        time_steps = self.eqn_config.time_steps
        dims = self.eqn_config.dim
        ...
        dataset = dataset.batch(self.eqn_config.batch_size)
        self.solver.fit(x=dataset, epochs=nr_epochs, callbacks=[history])
        self.solver.no_net.save_weights(checkpoint_path)
        # return history
        return history
